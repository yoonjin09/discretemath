    ...
    */
    ...
    for(x_l =1; x_l<length; x_l++){
        for(y_l =1; y_l<length; y_l++){
            ...
            if(confirm_num >=0 && confirm_num<9){ //만약 그 칸에 숫자가 들어있다면 SUBSETS 을 준비한다.
                intset * s = intset_alloc() ;
                intset_add(s,1); intset_add(s,2); intset_add(s,3); intset_add(s,4);
                intset_add(s,5); intset_add(s,6); intset_add(s,7); intset_add(s,8); //먼저 다 ADD를 해놓고 조건에 따라 뺀다
                
                if(x_l == 1){ // 만약 맨 윗줄이라면 윗부분이 없다.
                    intset_remove(s,1);
                    intset_remove(s,2);
                    intset_remove(s,3);
                }
                if(x_l == length){ //만약 맨 아래라면 아래부분이 없다.
                    intset_remove(s,6);
                    intset_remove(s,7);
                    intset_remove(s,8);
                }

                if(y_l == 1){ // 만약 맨 왼쪽이라면 윗부분이 없다.
                    intset_remove(s,1);
                    intset_remove(s,4);
                    intset_remove(s,6);
                }
                if(y_l == length){ //만약 맨 오른쪽이라면 아래부분이 없다.
                    intset_remove(s,3);
                    intset_remove(s,5);
                    intset_remove(s,8);
                }
